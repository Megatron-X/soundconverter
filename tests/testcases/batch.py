...
class BatchUtils(unittest.TestCase):
    ...
    def test_prepare_files_list(self):
        ...
        self.assertListEqual(subdirectories, ["b/"])
    ...
    def test_prepare_files_list_multiple(self):
        ...
        self.assertIn(
            "file://" + os.path.realpath("tests/test%20data/audio/a.wav"), parsed_files
        )
        self.assertIn(
            "file://"
            + urllib.parse.quote(
                os.path.realpath("tests/test data/audio/strângë chàrs фズ.wav")
            ),
            parsed_files,
        )
    ...
    def test_two_dirs(self):
        ...
        expected_dirs = ["audio/b/", "audio/", "audio/", "empty/", "empty/b/"]
        expected_dirs.sort()
        subdirectories.sort()
    ...
    def test_single_dir_depth_input(self):
        ...
        expected_files = [
            ...
            "file://"
            + urllib.parse.quote(
                os.path.realpath("test data/audio/strângë chàrs фズ.wav")
            ),
    ...
    def test_absolute(self):
        settings["recursive"] = True
        parsed_files, subdirectories = prepare_files_list(
            [os.path.realpath("tests/test data")]
        )
    ...
    def test_files(self):
        settings["recursive"] = True
        parsed_files, subdirectories = prepare_files_list(
            ["tests/test data/audio/b/c.mp3", "tests/test data/audio/a.wav"]
        )
    ...
    def test_validate_args(self):
        ...
        self.assertTrue(
            validate_args(
                {"main": "batch", "output-path": ".", "format": "mp3", "quality": 5}
            )
        )
        # input and output
        self.assertFalse(
        ...
        # formats
        self.assertFalse(
            validate_args({"main": "batch", "output-path": ".", "quality": 5})
        )
        self.assertFalse(
            validate_args(
                {
        ...
        self.assertFalse(
            validate_args(
                {"main": "batch", "output-path": ".", "format": "wav", "quality": 13}
            )
        )
        self.assertFalse(
            validate_args(
                {"main": "batch", "output-path": ".", "format": "flac", "quality": 20}
            )
        )
        self.assertFalse(
            validate_args(
                {"main": "batch", "output-path": ".", "format": "ogg", "quality": 20}
            )
        )
    ...
    def test_use_memory_gsettings_cbr(self):
        ...
        self.assertEqual(gio_settings.get_string("mp3-mode"), "cbr")
        self.assertEqual(gio_settings.get_string("output-mime-type"), "audio/mpeg")
        self.assertEqual(gio_settings.get_int("mp3-cbr-quality"), 320)

    def test_use_memory_gsettings_default_mp3_mode(self):
        use_memory_gsettings(
            {"output-path": ".", "main": "batch", "format": "mp3", "quality": "5"}
        )
        gio_settings = get_gio_settings()
        self.assertEqual(gio_settings.get_string("mp3-mode"), "vbr")
        self.assertEqual(gio_settings.get_string("output-mime-type"), "audio/mpeg")
        self.assertEqual(gio_settings.get_int("mp3-vbr-quality"), 5)

    def test_use_memory_gsettings_ogg(self):
        use_memory_gsettings(
            {"output-path": ".", "main": "batch", "format": "ogg", "quality": "0.5"}
        )
        gio_settings = get_gio_settings()
        self.assertEqual(gio_settings.get_string("output-mime-type"), "audio/x-vorbis")
        self.assertEqual(gio_settings.get_double("vorbis-quality"), 0.5)
